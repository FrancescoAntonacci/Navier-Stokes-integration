...
def fluid_evolution(u, rho, p,Amus, dt, dx, mu, zeta, kappa, gamma,charge_mass_ratio,polar):
    ...
    """
    u = ns(u, rho, p,Amus, mu, zeta, dt, dx)
...
def compute_B(Amu, dx):
    ...
    # Compute components using np.gradient
    dAz_dy, dAz_dx, dAz_dz = np.gradient(Az, dx, dx, dx)
    dAy_dy, dAy_dx, dAy_dz = np.gradient(Ay, dx, dx, dx)
    dAx_dy, dAx_dx, dAx_dz = np.gradient(Ax, dx, dx, dx)
    
    B =[dAz_dy - dAy_dz,dAx_dz - dAz_dx,dAy_dx - dAx_dy]     
    return np.array(B)
...
def EM_update(Amu, rho, u, charge_mass_ratio, polar, dx, dt, c, mu0):
    ...
    """
    # --- Compute source currents ---
    J_vector = rho * charge_mass_ratio * u          # 3-vector current
    J_scalar = rho * charge_mass_ratio              # scalar charge density
    E_field = compute_E(Amu, dx, dt)               # E at t^n

    Jmu = np.zeros_like(Amu)
    Jmu[0, 1, :, :, :] = J_scalar
    Jmu[1:3, 1, :, :, :] = J_vector + rho * polar * E_field
...
rho = 1 + 0 * np.exp(-(X**2 + Y**2 + Z**2)/(2*(Nx/10)**2))
...
while t < t1:
    ...
    Jmus,Amus=EM_update(Amus,Jmus,u,charge_mass_ratio,polar,dx,dt,c,mu0)
