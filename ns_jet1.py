...
def ns(u, rho, p, mu, zeta, dt, dx):
    ...
    dudt = -conv - grad_p / rho + mu * lap_u / rho + (zeta + mu/3) * grad_div_u / rho
...
# --------------------------------------------------------------------
Nx = 60              # Grid size in each direction
dx = 1            # Spatial step [?]
...
# Create Gaussian profile in y-z plane
Yc = Y[jx, jy, jz]  # center coordinates
Zc = Z[jx, jy, jz]
r2 = Yc**2 + Zc**2
u[0, jx, jy, jz] =0.1* np.exp(-r2 / (2 * sigma_y * sigma_z))
...
while t < t1:
    ...
    u, rho, p = temporal_evolution(u, rho, p, dt, dx, mu, zeta, kappa, gamma)  # evolve fields
    u[0, jx, jy, jz] = np.exp(-r2 / (2 * sigma_y * sigma_z))
