...
U = nu*(4*np.cos(THETA)*(1+c-np.cos(THETA)) - 2*np.sin(THETA)**2) / (RR*(1+c-np.cos(THETA))**2)
V = -nu*(2*np.sin(THETA)) / (RR*(1+c-np.cos(THETA)))

VX = U*np.sin(THETA)*np.cos(PHI) + V*np.cos(THETA)*np.cos(PHI)
VY = U*np.sin(THETA)*np.sin(PHI) + V*np.cos(THETA)*np.sin(PHI)
VZ = U*np.cos(THETA) - V*np.sin(THETA)

u = np.stack([VX, VY, VZ], axis=0)
